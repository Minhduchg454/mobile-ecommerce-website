...
const updateProductVariationRating = async (productVariationId) => {
  ...
  /*
        ...
        lượt 3: sum = 8 + 4 = 12
    */
  const totalStars = previews.reduce(
    (sum, review) => sum + review.previewRating,
    0
  );
  ...
  const averageRating =
    totalRating > 0 ? (totalStars / totalRating).toFixed(1) : 0;
...
exports.createPreview = asyncHandler(async (req, res) => {
  ...
  if (missingFields.length > 0) {
    return res.status(400).json({
      success: false,
      mes: `Missing required field(s): ${missingFields.join(", ")}`,
    });
  }
...
exports.getPreviews = asyncHandler(async (req, res) => {
  ...
  if (!userId)
    return res.status(400).json({ success: false, mes: "Missing userId" });
...
exports.deletePreview = asyncHandler(async (req, res) => {
  ...
  return res.json({
    success: !!deleted,
    mes: deleted ? "Preview deleted" : "Delete failed",
  });
});
