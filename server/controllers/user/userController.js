...
const register = asyncHandler(async (req, res) => {
  ...
  if (!/^[0-9a-fA-F]{24}$/.test(roleId)) {
    return res.status(400).json({
      success: false,
      mes: "Invalid roleId format. Must be a 24-character hex string.",
    });
  }
  ...
  if (existedAccount) {
    return res
      .status(400)
      .json({ success: false, mes: "Account already exists with this email" });
  }
...
const login = asyncHandler(async (req, res) => {
  ...
  let roleValue = 0;
  let roleName =
    user.roleId && user.roleId.roleName ? user.roleId.roleName : "";
  if (roleName === "admin") {
    roleValue = 1945;
  }
...
const getCurrent = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, mes: "Server error", error: err.message });
  }
});
