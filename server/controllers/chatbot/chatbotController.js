  ...
  try {
    ...
    if (response.text()) {
      ...
      if (functionCall && availableFunctions[functionCall.name]) {
        ...
        console.log(
          `Gemini muốn gọi hàm: ${functionCall.name} với đối số: `,
          functionArgs
        );
        ...
        return res.status(500).json({
          error:
            "Gemini yêu cầu một hàm không hợp lệ hoặc không được định nghĩa.",
        });
      }
    } else {
      ...
      return res
        .status(500)
        .json({ error: "Xin lỗi, tôi không thể trả lời câu hỏi này." });
    }
  } catch (error) {
    ...
    if (
      ...
      error.response.promptFeedback.blockReasons
    ) {
    ...
    res
      .status(500)
      .json({ error: "Xin lỗi, đã có lỗi xảy ra khi xử lý tin nhắn của bạn." });
  }
};
