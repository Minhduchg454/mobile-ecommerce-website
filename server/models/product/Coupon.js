...
const couponSchema = new mongoose.Schema({
    ...
    expirationDate: { // Theo biểu đồ: expirationDate: Date
        type: Date,
        required: true // Ngày hết hạn là bắt buộc
    },
    // Bạn có thể thêm các trường bổ sung thường có cho coupon như:
    couponCode: { // Mã code thực tế để người dùng nhập (ví dụ: 'SALE20')
        type: String,
        unique: true,
        trim: true,
    ...
    isActive: { // Trạng thái hoạt động của coupon
        type: Boolean,
        default: true
    },
    usageLimit: { // Số lần coupon có thể được sử dụng (tổng cộng)
        type: Number,
