...
if (isNotProduction) {
  ...
  router.post('/', (req, res) => res.status(403).json({ success: false, mes: 'POST /api/role is disabled in production!' }));
  router.put('/:id', (req, res) => res.status(403).json({ success: false, mes: 'PUT /api/role is disabled in production!' }));
  router.delete('/:id', (req, res) => res.status(403).json({ success: false, mes: 'DELETE /api/role is disabled in production!' }));
}
