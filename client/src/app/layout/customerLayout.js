...
export const CustomerLayout = () => {
  ...
  const name =
    `${current?.userLastName || ""} ${current?.userFirstName || ""}`.trim() ||
    "Khách hàng";
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          to: `/${path.CUSTOMER}/${customerId}/${path.C_SUCCEEDED_ORDER}`,
        },
      ],
    },
    {
        ...
        <div className="sticky top-0 z-10 mb-4 flex justify-between items-center bg-app-bg/5 backdrop-blur-sm gap-2 rounded-3xl">
          ...
          {isShowSearchBox && (
            <div className="glass border rounded-3xl relative flex px-2 py-1.5 w-full lg:w-[400px] focus-within:border-[2px] focus-within:border-input-fc transition-all duration-200 shadow-md z-10">
              ...
              <button
                type="button"
                className="absolute inset-y-0 right-5 top-1/2 -translate-y-1/2 flex p-0.5 border w-5 h-5 bg-black/60 shadow-md rounded-full items-center justify-center text-white"
                onClick={() => {
                  setSearchTerm("");
                  const params = new URLSearchParams(location.search);
                  params.delete("s");
