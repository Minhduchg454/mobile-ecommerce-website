    ...
    <div className="min-h-screen flex items-center justify-center bg-[#00afff] relative overflow-hidden">
      ...
      <div className="w-full max-w-md p-8 bg-white rounded-2xl shadow-2xl flex flex-col items-center z-10">
        ...
        <form className="w-full flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="bg-cyan-600 hover:bg-cyan-700 text-white py-2 rounded font-semibold"
          >
        ...
        <Link
          to={`/${path.HOME}`}
          className="text-gray-600 text-sm hover:text-cyan-600 hover:underline mt-2"
        >
          Quay lại trang chủ
        </Link>
      </div>
...
//         ...prev,
//         { name: "firstname", mes: "Không được để trống." },
//       ]);
//     }
...
//       setRegisterInvalidFields((prev) => [
//         ...prev,
//         { name: "lastname", mes: "Không được để trống." },
//       ]);
//     }
...
//       setRegisterInvalidFields((prev) => [
//         ...prev,
//         { name: "email", mes: "Không được để trống." },
//       ]);
//     }
...
//       setRegisterInvalidFields((prev) => [
//         ...prev,
//         { name: "mobile", mes: "Không được để trống." },
//       ]);
//     }
...
//       setRegisterInvalidFields((prev) => [
//         ...prev,
//         { name: "password", mes: "Không được để trống." },
//       ]);
//     }
...
//     if (!payload.email || payload.email.trim() === "") {
//       invalids++;
//       newInvalidFields.push({ name: "email", mes: "Không được để trống." });
//     } else if (!emailRegex.test(payload.email)) {
//       invalids++;
//       newInvalidFields.push({ name: "email", mes: "Email không hợp lệ." });
//     }

//     if (!payload.password || payload.password.trim() === "") {
//       invalids++;
//       newInvalidFields.push({ name: "password", mes: "Không được để trống." });
...
//     setRegisterInvalidFields(newInvalidFields);
//     return invalids === 0;
//   }, [payload]);
...
//         showModal({
//           isShowModal: true,
//           modalChildren: <div>Đang xử lý...</div>,
//         })
//       );
...
//           const usedErr = prev.find(
//             (f) => f.name === "email" && f.mes.includes("được sử dụng")
//           );
//           return [
...
//         setRegisterInvalidFields((prev) => {
//           const usedErr = prev.find(
//             (f) => f.name === "mobile" && f.mes.includes("được sử dụng")
//           );
//           return [
...
//       let ignore = false;
//       apiCheckEmailExists(debouncedEmail)
//         .then((res) => {
//           if (!ignore) {
//             const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
...
//       let ignore = false;
//       apiCheckMobileExists(debouncedMobile)
//         .then((res) => {
//           if (!ignore) {
//             const phoneRegex = /^0\d{9}$/;
//             if (res && res.data && typeof res.data.exists !== "undefined") {
...
//   // Cập nhật useEffect cho kiểm tra real-time trong chế độ đăng ký
//   useEffect(() => {
//     if (isRegister) {
//       const newInvalidFields = [];
//       const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
...
//       // Kiểm tra mobile
//       if (!payload.mobile || payload.mobile.trim() === "") {
//         newInvalidFields.push({ name: "mobile", mes: "Không được để trống." });
//       } else if (!phoneRegex.test(payload.mobile)) {
//         newInvalidFields.push({
//           name: "mobile",
...
//     registerInvalidFields.filter((f) =>
//       ["firstname", "lastname", "email", "mobile", "password"].includes(f.name)
//     ).length === 0;
...
//     payload.email.trim() &&
//     payload.password.trim() &&
//     registerInvalidFields.filter((f) => ["email", "password"].includes(f.name))
//       .length === 0;
...
//                   onBlur={handleBlur}
//                   placeholder="Họ"
//                   className="pl-10 py-2 rounded border border-gray-200 bg-white text-gray-800 placeholder-gray-400 w-full focus:ring-2 focus:ring-cyan-200 outline-none"
//                 />
//                 {registerInvalidFields.find((f) => f.name === "firstname") &&
//                   (payload.firstname ||
//                     registerTouched.firstname ||
//                     registerSubmitted) && (
//                     <span className="text-red-500 text-xs">
//                       {
//                         registerInvalidFields.find(
//                           (f) => f.name === "firstname"
//                         ).mes
//                       }
//                     </span>
...
//                   className="pl-10 py-2 rounded border border-gray-200 bg-white text-gray-800 placeholder-gray-400 w-full focus:ring-2 focus:ring-cyan-200 outline-none"
//                 />
//                 {registerInvalidFields.find((f) => f.name === "lastname") &&
//                   (payload.lastname ||
//                     registerTouched.lastname ||
//                     registerSubmitted) && (
//                     <span className="text-red-500 text-xs">
//                       {
//                         registerInvalidFields.find((f) => f.name === "lastname")
...
//             />
//             {emailError.length > 0 &&
//               (payload.email || registerTouched.email || registerSubmitted) && (
//                 <span className="text-red-500 text-xs">
//                   {emailError.find((f) => f.mes.includes("được sử dụng"))
//                     ?.mes || emailError[0].mes}
//                 </span>
//               )}
//           </div>
//           {isRegister && (
...
//                 (payload.mobile ||
//                   registerTouched.mobile ||
//                   registerSubmitted) && (
//                   <span className="text-red-500 text-xs">
//                     {mobileError.find((f) => f.mes.includes("được sử dụng"))
//                       ?.mes || mobileError[0].mes}
//                   </span>
//                 )}
//             </div>
//           )}
...
//               onBlur={handleBlur}
//               placeholder="Mật khẩu"
//               className="pl-10 pr-10 py-2 rounded border border-gray-200 bg-white text-gray-800 placeholder-gray-400 w-full focus:ring-2 focus:ring-cyan-200 outline-none"
//             />
//             <span
//               className="absolute right-3 top-3 text-gray-400 cursor-pointer"
//               onClick={() => setShowPassword((v) => !v)}
...
//             {registerInvalidFields.find((f) => f.name === "password") &&
//               (isRegister
//                 ? payload.password ||
//                   registerTouched.password ||
//                   registerSubmitted
//                 : payload.password || touched.password || submitted) && (
//                 <span className="text-red-500 text-xs">
//                   {registerInvalidFields.find((f) => f.name === "password").mes}
//                 </span>
//               )}
//           </div>
//           <button
//             type="submit"
//             className={`w-full py-2 mt-2 text-white font-semibold rounded shadow transition-all ${
//               isRegister
//                 ? isRegisterFormValid
